...
import { MapPin, Globe, ArrowLeft, AlignLeft, Image as ImageIcon } from 'lucide-react';
import Link from 'next/link';

// Налаштування клієнта прямо у файлі для гарантії стабільності
const client = createClient({
  projectId: '5tbxcnx4',
  dataset: 'production',
  apiVersion: '2024-03-03',
  useCdn: false, // Ставимо false, щоб бачити нові дані миттєво
});

// Змушуємо Next.js не кешувати цю сторінку (щоб нові мітки з'являлися відразу)
...
export default async function LocationPage({ params }: { params: { id: string } }) {
  ...
  // Отримуємо дані про локацію, включаючи розгорнуті дані категорії
  const query = `*[_type == "location" && (slug.current == "${id}" || _id == "${id}")][0]{
    title,
    address,
    description,
    link,
  ...
  if (!location) {
    return (
      <div className="h-screen bg-slate-950 text-white flex flex-col items-center justify-center space-y-6 font-black italic uppercase tracking-tighter">
        <h1 className="text-3xl text-center px-4">Локацію "{id}" <br/>не знайдено в базі</h1>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6 md:p-12 lg:p-24 flex justify-center selection:bg-yellow-400 selection:text-black">
      <div className="max-w-4xl w-full space-y-10">
        ...
        {/* Головне зображення (якщо є) */}
        {location.imageUrl && (
          <div className="w-full aspect-video rounded-[40px] overflow-hidden border border-white/10">
            <img 
